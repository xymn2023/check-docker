...
install_release() {
  for command in python3 docker systemctl sha256sum; do
    command -v "$command" >/dev/null || { echo "缺少依赖：$command；请参阅 README.md"; return 1; }
  done
  python3 -c 'import sys; assert sys.version_info >= (3,10), "需要 Python 3.10+"'
  docker info >/dev/null
  docker compose version
  [[ -f "$SOURCE_DIR/SHA256SUMS" ]] || { echo '缺少 SHA256SUMS，请使用完整发行包'; return 1; }
  ...
  cp "$SOURCE_DIR/"{autoupdate_bot.py,core.py,admin.py,requirements.txt} "$release/"
...
require_root
if [[ "${1:-}" == '--install' ]]; then install_release; exit; fi
while true; do
